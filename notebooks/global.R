...
knitr::opts_chunk$set(echo = TRUE) #What does this do? 

#Steps for loading in the ACS data in via tidycensus
...
#Input the ACS data for the two metro areas we want to assess: 
#These are their GEOID as coded by the US Census
...
#Input the ACS data for the State of Tennessee
...
#Combine each ACS input
acs5_2019 <- bind_rows(acs5_2019_dp_nash_memph, 
                       acs5_2019_dp_tn, 
                       acs5_19_dtl_tn, 
                       acs5_19_dtl_nash_memph
                       )

#Organize data for use in the dash
...
#Step 3 - Populate the pop_group column
acs5_2019_filter <- acs5_2019_filter %>% 
...
#Step 5 - Create subgroups for age categories
acs5_2019_filter <- acs5_2019_filter %>% 
  mutate(
...
#More combining of these age groups now that a categorical variable has been assigned to each
...
#Simplifing Education categories
acs5_2019_filter <- acs5_2019_filter %>% 
  mutate(
    statistic = case_when(
...
#Gather population denominators
...
#Final output of data into a csv:
write.csv(acs5_2019_filter, '../data/acs5_2019_filter.csv', row.names = FALSE)

#censusdata <- read_csv('../data/acs5_2019_filter.csv')

#People 3 Theme Settings
...
#Create a sample company template:
statistic_group <- c(
  "Sex and Age",
  "Sex and Age",
  "Sex and Age",
...
profile <- profile %>% 
  rename('Company Name' = companyname, 
         'Statistic Group' = statistic_group, 
         'Statistic' = statistic, 
         'Sex' = sex, 
         'Nbr Employees' = employees)
...
companydata <- companydata %>% 
  rename('NAME' = Company.Name, 'statistic_group' = Statistic.Group, 'statistic' = Statistic, 'sex' = Sex, 'estimate' = Nbr.Employees)

companydata <- companydata %>% 
  mutate('geoid' = 9999999, .before = NAME) %>% 
...
#Data manipulation for UI output
censusdata <- censusdata %>%
  mutate_at('NAME',str_replace, "Memphis, TN-MS-AR Metro Area", "Memphis MSA")%>%
  mutate_at('NAME',str_replace, "Nashville-Davidson--Murfreesboro--Franklin, TN Metro Area", "Nashville MSA")%>%
  mutate_at('age_group', str_replace, "Baby Boom", "Baby Boomer") %>%
  rename('location' = NAME) %>%
  filter(statistic_group != "Veteran Status" & statistic_group != "Disability Status")

write.csv(censusdata, '../data/censusdata.csv', row.names = FALSE)

#Dashboard Input Choices
location_choices <- censusdata %>% 
  distinct(location) %>% 
  deframe()
plot_choices <- censusdata %>% 
  distinct(statistic_group) %>% 
  deframe()
